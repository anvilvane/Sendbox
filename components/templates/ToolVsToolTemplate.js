  ...
  return (
    <div className="overflow-x-auto rounded-lg border border-gray-200">
      <table className="w-full border-collapse">
        <thead>
          <tr>
            <th className="text-left px-3 py-3 text-[11px] font-semibold text-[#9ca3af] uppercase tracking-wider bg-gray-50 border-b border-gray-200 w-[34%]">Feature</th>
            <th className="text-left px-3 py-3 bg-gray-50 border-b border-gray-200 w-[33%]">
              <span className="flex items-center gap-1.5">
                <Image src={toolALogo} alt={toolAName} width={64} height={16} className="h-4 w-auto rounded" />
                <span className="text-[12px] font-semibold text-[#6b7280]">{toolAName}</span>
              </span>
            </th>
            <th className="text-left px-3 py-3 bg-gray-50 border-b border-gray-200 w-[33%]">
              <span className="flex items-center gap-1.5">
                <Image src={toolBLogo} alt={toolBName} width={64} height={16} className="h-4 w-auto rounded" />
                <span className="text-[12px] font-semibold text-[#6b7280]">{toolBName}</span>
              </span>
            </th>
          </tr>
        </thead>
        <tbody>
          {categories.map((category) => (
            <React.Fragment key={category}>
              {categories.length > 1 && (
                <tr>
                  <td colSpan={3} className="px-3 py-2 text-[11px] font-semibold uppercase tracking-wider text-[#9ca3af] bg-white border-b border-gray-100 border-t border-gray-100">
                    {category}
                  </td>
                </tr>
              )}
              {rows.filter(r => r.category === category).map((row, i) => (
                <tr key={i} className="border-b border-gray-100 last:border-0">
                  <td className="px-3 py-3 text-[13px] font-medium text-black">{row.feature}</td>
...
export default function ToolVsToolTemplate({ data }) {
  const d = data;
  const readTime = Math.ceil(
    (d.sections.reduce((a, s) => a + s.content.split(/\s+/).length, 0)
    + d.faqs.reduce((a, f) => a + f.answer.split(/\s+/).length, 0)
    + d.comparisonTable.length * 10) / 200
  );
          ...
          <div className="flex-1 min-w-0 space-y-14">
            ...
            <div id="comparison-table" className="scroll-mt-24">
              <h2 className="text-[1.375rem] md:text-[1.5rem] font-semibold text-black tracking-tight leading-[1.2] mb-5">
                {d.toolA.name} vs {d.toolB.name}: Feature-by-Feature
              </h2>
              <TwoWayTable
            ...
            {/* Pros & Cons (head-to-head only, no Sendbox overlay) */}
            ...
            <ExternalSources tools={[d.toolA.name.toLowerCase().replace(/\./g, ''), d.toolB.name.toLowerCase().replace(/\./g, '')]} />
          </div>
